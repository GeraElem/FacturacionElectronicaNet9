...
using FacturaElectronica.WSFEV1;

namespace FacturaElectronica.ConsumirWSFE
{
    public class SolicitarCAE
    {
        const string strUrlWsfev1 = "https://wswhomo.afip.gov.ar/wsfev1/service.asmx?WSDL"; //testing
        ...
        public FEResponse solicitar(CabRequest cabReq, DetRequest detReq)
        {
            ...
            if (response.FeDetResp[0].Observaciones != null)
            {
                ...
                respuestaDetalle.observaciones = new ObsResponse[1]; //= new ObsResponse[numero];
                for (int i = 0; i < response.FeDetResp[0].Observaciones.Length; i++)
                {
                    respuestaDetalle.observaciones[i].code = response.FeDetResp[0].Observaciones[i].Code;
                    respuestaDetalle.observaciones[i].msg = response.FeDetResp[0].Observaciones[i].Msg;
                }
            }
        ...
        //solicita dos objetos - cabecera y detalle - para armar los respectivos objetos del ws
        //devuelve un objeto respuesta, el cual contiene cabecera, detalle (con el CAE), errores y eventos con la información a manipular
        public FEResponse solicitarMultiples(CabRequest cabReq, List<DetRequest> detReq)
        {
            try
            {
                //solicito objeto de autenticación
                ...
                for (int i = 0; i < response.FeDetResp.Count(); i++)
                {
                    ...
                    if (response.FeDetResp[i].Observaciones != null)
                    {
                        ...
                        respuestaDetalle.observaciones = new ObsResponse[numero + 1]; //= new ObsResponse[numero];
                        for (int x = 0; x < response.FeDetResp[i].Observaciones.Length; x++)
                        {
                            respuestaDetalle.observaciones[x].code = response.FeDetResp[i].Observaciones[x].Code;
                            respuestaDetalle.observaciones[x].msg = response.FeDetResp[i].Observaciones[x].Msg;
                        }
                    }
